...
import com.shawen.myOwnRPC.common.*;
...
public class RpcMessageDecoder extends LengthFieldBasedFrameDecoder {
    ...
    protected Object decode(ChannelHandlerContext ctx, ByteBuf in) throws Exception {
        ...
        if (decoded instanceof ByteBuf) {
            ...
            // 如果解码的结果是一个ByteBuf对象且有足够的可读字节，调用decodeFrame方法进行进一步解析。
    ...
    private Object decodeFrame(ByteBuf in) {
        ...
        if (bodyLength > 0) {
            ...
            Serializer serializer = Serializer.getSerializerByCode(codecType);

            if (messageType == RpcConstants.REQUEST_TYPE) {
                RPCRequest tmpValue = (RPCRequest)serializer.deserialize(bs, messageType);
                rpcMessage.setData(tmpValue);
            } else if (messageType == RpcConstants.RESPONSE_TYPE){
                RPCResponse tmpValue = (RPCResponse) serializer.deserialize(bs,messageType);
                rpcMessage.setData(tmpValue);
            }
        }
        return rpcMessage;
